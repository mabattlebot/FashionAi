// Global variables
let currentStep = 1;
let userProfile = JSON.parse(localStorage.getItem('fashionAI_profile') || '{"personality": "", "colors": "", "style": ""}');
let capturedImage = null;
let stream = null;
let currentFacingMode = 'user';
let savedOutfits = JSON.parse(localStorage.getItem('fashionAI_outfits') || '[]');
...
function deleteOutfit(index, event) {
    ...
    if (confirm('Are you sure you want to delete this outfit?')) {
        ...
        localStorage.setItem('fashionAI_outfits', JSON.stringify(savedOutfits));
...
function clearProfile() {
    if (confirm('Are you sure you want to clear your style profile? You will need to complete the survey again.')) {
        ...
        localStorage.removeItem('fashionAI_profile');
        updateProfileDisplay();
...
function selectOption(personality) {
    ...
    localStorage.setItem('fashionAI_profile', JSON.stringify(userProfile));
    enableNextButton();
}
...
async function initializeCamera() {
    try {
        ...
        const hasPermission = localStorage.getItem('fashionAI_cameraPermission') === 'granted';
        
        if (!hasPermission) {
            // Request permission once
            const permissionStream = await navigator.mediaDevices.getUserMedia({ video: true });
            permissionStream.getTracks().forEach(track => track.stop()); // Stop immediately
            localStorage.setItem('fashionAI_cameraPermission', 'granted');
        }
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    console.log('FashionAI app loaded successfully!');
